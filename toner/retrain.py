...
def retrain_model(file_path):
    model = load_model("my_model3.h5", compile= False)
    #model = build_model2()
    keras.utils.plot_model(model, show_shapes=True)
    optimizer = "adam"
    model.compile(loss = compute_loss, optimizer = optimizer, metrics = [])
    model.fit_generator(generate_xy_batch(), steps_per_epoch=10, epochs=10, verbose=1, validation_data=generate_xy_batch(), validation_steps=20, class_weight=None, max_queue_size=10, workers=1, use_multiprocessing=False, shuffle=False, initial_epoch=0)
    model.save(file_path)

retrain_model("my_model222")
